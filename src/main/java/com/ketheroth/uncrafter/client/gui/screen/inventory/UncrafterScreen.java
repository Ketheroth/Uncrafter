...
public class UncrafterScreen extends AbstractContainerScreen<UncrafterContainer> {
	...
	public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTicks) {
		...
		if (this.menu.isInputLocked()) {
			...
			this.blit(poseStack, this.leftPos + 51, this.topPos + 51, 176, 0, 3, 3);
		}
	}
	...
	protected void renderLabels(PoseStack poseStack, int x, int y) {
		...
		this.font.draw(poseStack, this.playerInventoryTitle.getContents(), this.inventoryLabelX, this.inventoryLabelY, 4210752);
	}
	...
	protected void renderBg(PoseStack poseStack, float partialTicks, int x, int y) {
		...
		for (int i = 0; i < selected.length; i++) {
			if (selected[i]) {
				this.blit(poseStack, this.leftPos + 93 + i%3 *18, this.topPos + 17 + i/3*18, 176, 3, 16, 16);
			}
		}
	}
